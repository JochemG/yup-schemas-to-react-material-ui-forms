...
export function RecursiveFormFromSchema({namePath, schema, value, schemaMetaData}: FieldComponentProps) {
    const Component = MAP_FIELD_TYPE_TO_COMPONENT[schema.type];
    return <>
        {Component ?
            <Component schema={schema} namePath={namePath} value={value} schemaMetaData={schemaMetaData}/>:
